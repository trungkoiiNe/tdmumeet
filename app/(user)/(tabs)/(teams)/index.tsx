            ...
            <View style={styles.teamStats}>
              ...
              {(isCreator || isMember) && (
                <TouchableOpacity
                  ...
                  onPress={() => onCopy(item.inviteCode)}
                >
                  <MaterialCommunityIcons
                    name="key-variant"
                    size={16}
...
export default function TeamsScreen() {
  ...
  const filteredTeams = teams?.filter((team) => {
    ...
    return (
      team.tags &&
      team.tags.some((tag: string) => selectedFilterTags.includes(tag))
    );
  });
  ...
  const handleCreateOrUpdate = async () => {
    ...
    try {
      ...
      if (editingTeam) {
        await updateTeam(editingTeam.id, { ...formData, tags: processedTags });
        setAlert({
          visible: true,
          title: "Success",
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient colors={["#32409A", "#44ADE2"]} style={styles.header}>
        <Text style={styles.headerTitle}>Teams</Text>
        <MultiSelect
          items={availableTags}
          IconRenderer={MaterialCommunityIcons}
          uniqueKey="id"
          onSelectedItemsChange={setSelectedFilterTags}
          selectedItems={selectedFilterTags}
